...
# Now import the aggregators as functions
from consistent_agents.metrics.aggregators.pairwise import pairwise
from consistent_agents.metrics.aggregators.entropy import _semantic_clustering
...
class TestPairwiseAggregator:
    ...
    def test_pairwise_aggregator(self):
        """Test that pairwise function computes average agreement correctly."""
        ...
        total_agreement, total_pairs = pairwise(outputs, mock_agreement)
        result = total_agreement / total_pairs if total_pairs > 0 else 1.0
    ...
    def test_pairwise_aggregator_single_output(self):
        """Test pairwise function with single output returns 1.0."""
        mock_agreement = MockAgreementFunction()
        
        outputs = ["Single output"]
        result = pairwise(outputs, mock_agreement)
...
class TestEntropyAggregator:
    ...
    def test_entropy_one_sentence_repeated_cluster_number_one(self):
        ...
        # Get clusters by calling the function directly
        clusters = _semantic_clustering(outputs, mock_agreement, question=None, threshold=0.5)
